...
(Generated by GPT Agent, may not be accurate)
...
- Example:
  ...
  image_data = Data2DScattering('image.tiff')
  ```
...
- Example:
  ...
  protocol = Protocols.circular_average_q2I()
  protocol.run(image_data, output_dir='results/')
  ```
...
```python
from SciAnalysis.XSAnalysis.Data import Data2DScattering
image = Data2DScattering('sample.tiff')
image.show()
```
...
from SciAnalysis.XSAnalysis.Tools import Calibration
calib = Calibration()
calib.set_beam_center(250, 260)
calib.apply_mask('masks/Pilatus2M_CMS_5m-mask.xcf')
```

### 3. Running a Protocol
```python
from SciAnalysis.XSAnalysis import Protocols
protocol = Protocols.circular_average_q2I_fit()
protocol.run(image, output_dir='output/')
```

## Advanced Customization
### 1. Creating Custom Pipelines
Users can define custom analysis routines by subclassing existing protocols:
```python
from SciAnalysis.XSAnalysis import Protocols

class MyCustomProtocol(Protocols.circular_average_q2I):
    def run(self, data, output_dir, **kwargs):
        results = super().run(data, output_dir, **kwargs)
        print("Custom processing applied")
        return results
```

### 2. Modifying Existing Protocols
Edit `Protocols.py` to modify existing analysis routines.
